...
TYPED_TEST(GemmTest, TestGemmCPUGPU) {
  ...
  if (sizeof(Dtype) == 4 || CAFFE_TEST_CUDA_PROP.major >= 2) {
    ...
    for (int i = 0; i < 8; ++i) {
      EXPECT_EQ(Get<Mtype>(C.cpu_data()[i]), Get<Mtype>(result[i]));
    }
...
TYPED_TEST(GemmTest, TestGemvCPUGPU) {
  ...
  Dtype data[6] = {Get<Dtype>(1), Get<Dtype>(2), Get<Dtype>(3), Get<Dtype>(4), Get<Dtype>(5), Get<Dtype>(6)};
  ...
  if (sizeof(Dtype) == 4 || CAFFE_TEST_CUDA_PROP.major >= 2) {
    ...
    for (int i = 0; i < 2; ++i) {
      EXPECT_EQ(Get<Mtype>(y.cpu_data()[i]), Get<Mtype>(result_2[i]));
    }
    ...
    for (int i = 0; i < 3; ++i) {
      EXPECT_EQ(Get<Mtype>(x.cpu_data()[i]), Get<Mtype>(result_3[i]));
    }
