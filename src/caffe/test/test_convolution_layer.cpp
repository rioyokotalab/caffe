...
TYPED_TEST(ConvolutionLayerTest, Test0DConvolution) {
  ...
  const Blob<Dtype,Mtype>* weight = layer->blobs()[0].get();
  const Blob<Dtype,Mtype>* bias = layer->blobs()[1].get();
...
TYPED_TEST(ConvolutionLayerTest, TestSobelConvolution) {
  ...
  layer.reset(new ConvolutionLayer<Dtype,Mtype>(layer_param));
  layer->blobs().resize(1);
  layer->blobs()[0].reset(new Blob<Dtype,Mtype>(1, 1, 1, 3));
  Dtype* weights_2 = layer->blobs()[0]->mutable_cpu_data();
  weights_2[0] = Get<Dtype>(-1);
  weights_2[1] = Get<Dtype>(0);
  weights_2[2] = Get<Dtype>(1);
  layer->SetUp(sep_blob_bottom_vec, sep_blob_top_vec);
  layer->Forward(sep_blob_bottom_vec, sep_blob_top_vec);
