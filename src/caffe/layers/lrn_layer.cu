...
__global__ void LRNFillScale(const int nthreads, const Dtype* in,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    while (head < post_pad && head < channels) {
      accum_scale += Get<Mtype>(in[head * step]) * Get<Mtype>(in[head * step]);
      ++head;
    }
    ...
    while (head < channels) {
      accum_scale += Get<Mtype>(in[head * step]) * Get<Mtype>(in[head * step]);
      if (head - size >= 0) {
        accum_scale -= Get<Mtype>(in[(head - size) * step]) * Get<Mtype>(in[(head - size) * step]);
      }
      scale[(head - post_pad) * step] = Get<Dtype>(k + accum_scale * alpha_over_size);
      ++head;
    }
    // subtract only
...
__global__ void LRNComputeOutput(const int nthreads, const Dtype* in,
    const Dtype* scale, const Mtype negative_beta, Dtype* out) {
  CUDA_KERNEL_LOOP(index, nthreads) {
    out[index] = Get<Dtype>( Get<Mtype>(in[index]) * pow(Get<Mtype>(scale[index]), negative_beta) );
  }
}
