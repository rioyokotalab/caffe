...
Mtype SGDSolver<Dtype,Mtype>::GetLearningRate() {
  ...
  if (lr_policy == "fixed") {
    ...
    rate = this->param_.base_lr() *
        pow(Mtype(1) + this->param_.gamma() * this->iter_,
            - this->param_.power());
    ...
    rate = this->param_.base_lr() * pow(Mtype(1.) -
        (Mtype(this->iter_) / Mtype(this->param_.max_iter())),
        this->param_.power());
  } else if (lr_policy == "sigmoid") {
    rate = this->param_.base_lr() * (Mtype(1.) /
        (Mtype(1.) + exp(-this->param_.gamma() * (Mtype(this->iter_) -
          Mtype(this->param_.stepsize())))));
  } else {
...
void SGDSolver<Dtype,Mtype>::Normalize(int param_id) {
  ...
  switch (Caffe::mode()) {
  case Caffe::CPU: {
    caffe_scal<Dtype,Mtype>(net_params[param_id]->count(), accum_normalization,
  ...
  case Caffe::CPU: {
    if (local_decay) {
      if (regularization_type == "L2") {
        // add weight decay
        caffe_axpy<Dtype,Mtype>(net_params[param_id]->count(),
            local_decay,
            net_params[param_id]->cpu_data(),
            net_params[param_id]->mutable_cpu_diff());
      } else if (regularization_type == "L1") {
        caffe_cpu_sign<Dtype,Mtype>(net_params[param_id]->count(),
            net_params[param_id]->cpu_data(),
...
void SGDSolver<Dtype,Mtype>::ComputeUpdateValue(int param_id, Mtype rate) {
  ...
  switch (Caffe::mode()) {
  case Caffe::CPU: {
    caffe_cpu_axpby<Dtype,Mtype>(net_params[param_id]->count(), local_rate,
              net_params[param_id]->cpu_diff(), momentum,
              history_[param_id]->mutable_cpu_data());
    caffe_copy<Dtype,Mtype>(net_params[param_id]->count(),
  ...
  case Caffe::GPU: {
#ifndef CPU_ONLY
    caffe_gpu_axpby<Dtype,Mtype>(net_params[param_id]->count(), local_rate,
...
template <typename Dtype, typename Mtype>
void SGDSolver<Dtype,Mtype>::SnapshotSolverState(const string& model_filename) {
...
template <typename Dtype, typename Mtype>
void SGDSolver<Dtype,Mtype>::SnapshotSolverStateToBinaryProto(
  ...
  string snapshot_filename = Solver<Dtype,Mtype>::SnapshotFilename(".solverstate");
  LOG(INFO)
...
template <typename Dtype, typename Mtype>
void SGDSolver<Dtype,Mtype>::SnapshotSolverStateToHDF5(
    const string& model_filename) {
  string snapshot_filename =
      Solver<Dtype,Mtype>::SnapshotFilename(".solverstate.h5");
  ...
  for (int i = 0; i < history_.size(); ++i) {
    ostringstream oss;
    oss << i;
    hdf5_save_nd_dataset<Dtype,Mtype>(history_hid, oss.str(), *history_[i]);
  }
...
template <typename Dtype, typename Mtype>
void SGDSolver<Dtype,Mtype>::RestoreSolverStateFromBinaryProto(
...
template <typename Dtype, typename Mtype>
void SGDSolver<Dtype,Mtype>::RestoreSolverStateFromHDF5(const string& state_file) {
  ...
  for (int i = 0; i < history_.size(); ++i) {
    ostringstream oss;
    oss << i;
    hdf5_load_nd_dataset<Dtype,Mtype>(history_hid, oss.str().c_str(), 0,
...
void NesterovSolver<Dtype,Mtype>::ComputeUpdateValue(int param_id, Mtype rate) {
  ...
  case Caffe::CPU: {
    ...
    caffe_cpu_axpby<Dtype,Mtype>(net_params[param_id]->count(), Mtype(1) + momentum,
  ...
  case Caffe::GPU: {
    ...
    // compute update: step back then over step
    caffe_gpu_axpby<Dtype,Mtype>(net_params[param_id]->count(), Mtype(1) + momentum,
...
void AdaGradSolver<Dtype,Mtype>::ComputeUpdateValue(int param_id, Mtype rate) {
  ...
  case Caffe::CPU: {
    ...
    caffe_powx<Dtype,Mtype>(net_params[param_id]->count(),
        net_params[param_id]->cpu_diff(), Mtype(2),
        this->update_[param_id]->mutable_cpu_data());
    ...
    // prepare update
    caffe_powx<Dtype,Mtype>(net_params[param_id]->count(),
              this->history_[param_id]->cpu_data(), Mtype(0.5),
              this->update_[param_id]->mutable_cpu_data());

    caffe_add_scalar<Dtype,Mtype>(net_params[param_id]->count(),
              delta, this->update_[param_id]->mutable_cpu_data());

    caffe_div<Dtype,Mtype>(net_params[param_id]->count(),
    ...
    // scale and copy
    caffe_cpu_axpby<Dtype,Mtype>(net_params[param_id]->count(), local_rate,
        this->update_[param_id]->cpu_data(), Mtype(0),
        net_params[param_id]->mutable_cpu_diff());
    break;
  }
  case Caffe::GPU: {
#ifndef CPU_ONLY
    // compute square of gradient in update
    caffe_gpu_powx<Dtype,Mtype>(net_params[param_id]->count(),
        net_params[param_id]->gpu_diff(), Mtype(2),
        this->update_[param_id]->mutable_gpu_data());

    // update history
    caffe_gpu_add<Dtype,Mtype>(net_params[param_id]->count(),
        this->update_[param_id]->gpu_data(),
        this->history_[param_id]->gpu_data(),
        this->history_[param_id]->mutable_gpu_data());
    ...
    // prepare update
    caffe_gpu_powx<Dtype,Mtype>(net_params[param_id]->count(),
              this->history_[param_id]->gpu_data(), Mtype(0.5),
              this->update_[param_id]->mutable_gpu_data());

    caffe_gpu_add_scalar<Dtype,Mtype>(net_params[param_id]->count(),
              delta, this->update_[param_id]->mutable_gpu_data());

    caffe_gpu_div<Dtype,Mtype>(net_params[param_id]->count(),
    ...
    // scale and copy
    caffe_gpu_axpby<Dtype,Mtype>(net_params[param_id]->count(), local_rate,
        this->update_[param_id]->gpu_data(), Mtype(0),
        net_params[param_id]->mutable_gpu_diff());
#else
    NO_GPU;
#endif
...
void RMSPropSolver<Dtype,Mtype>::ComputeUpdateValue(int param_id, Mtype rate) {
  ...
  case Caffe::CPU:
    ...
    caffe_cpu_axpby(net_params[param_id] -> count(),
        Mtype(1-rms_decay), this->update_[param_id]->cpu_data(),
  ...
  case Caffe::GPU:
    ...
    caffe_gpu_axpby(net_params[param_id] -> count(),
        Mtype(1-rms_decay), this->update_[param_id]->gpu_data(),
...
void AdaDeltaSolver<Dtype,Mtype>::ComputeUpdateValue(int param_id, Dtype rate) {
  ...
  case Caffe::CPU: {
    ...
    caffe_powx<Dtype,Mtype>(net_params[param_id]->count(),
        this->update_[param_id]->cpu_data(), Mtype(0.5),
        this->update_[param_id]->mutable_cpu_data());

    // compute the update
    caffe_mul<Dtype,Mtype>(net_params[param_id]->count(),
        net_params[param_id]->cpu_diff(),
        this->update_[param_id]->cpu_data(),
        net_params[param_id]->mutable_cpu_diff());
    ...
    // apply learning rate
    caffe_cpu_scale<Dtype,Mtype>(net_params[param_id]->count(), local_rate,
        net_params[param_id]->cpu_diff(),
        net_params[param_id]->mutable_cpu_diff());
    break;
  }
  case Caffe::GPU: {
    ...
    // add delta to history to guard against dividing by zero later
    caffe_gpu_set<Dtype,Mtype>(net_params[param_id]->count(), delta,
    ...
    // jointly compute the RMS of both for update and gradient history
    caffe_gpu_powx<Dtype,Mtype>(net_params[param_id]->count(),
        this->update_[param_id]->gpu_data(), Mtype(0.5),
        this->update_[param_id]->mutable_gpu_data());

    // compute the update and copy to net_diff
    caffe_gpu_mul<Dtype,Mtype>(net_params[param_id]->count(),
        net_params[param_id]->gpu_diff(),
        this->update_[param_id]->gpu_data(),
        net_params[param_id]->mutable_gpu_diff());
...
void AdamSolver<Dtype,Mtype>::ComputeUpdateValue(int param_id, Dtype rate) {
  ...
  switch (Caffe::mode()) {
    ...
    // set update
    caffe_powx<Dtype,Mtype>(N,
        val_v->cpu_data(), Mtype(0.5),
        val_t->mutable_cpu_data());
    caffe_add_scalar<Dtype,Mtype>(N, eps_hat, val_t->mutable_cpu_data());
    caffe_div<Dtype,Mtype>(N,
        val_m->cpu_data(),
        val_t->cpu_data(),
        val_t->mutable_cpu_data());

    caffe_cpu_scale<Dtype,Mtype>(N, local_rate*correction,
        val_t->cpu_data(),
        net_params[param_id]->mutable_cpu_diff());
    break;
  }
  case Caffe::GPU: {
    ...
    // set update
    caffe_gpu_powx<Dtype,Mtype>(N,
        val_v->gpu_data(), Mtype(0.5),
        val_t->mutable_gpu_data());
    caffe_gpu_add_scalar<Dtype,Mtype>(N, eps_hat,
        val_t->mutable_gpu_data());
    caffe_gpu_div<Dtype,Mtype>(N,
        val_m->gpu_data(),
        val_t->gpu_data(),
        val_t->mutable_gpu_data());
