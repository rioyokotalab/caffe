...
// test nets.
#ifndef CPU_ONLY
void CopyLayers(caffe::Solver<float16,float16>* solver, const std::string& model_list) {
...
int train() {
  ...
  shared_ptr<caffe::Solver<float16,float16> >
    solver(caffe::GetSolver<float16,float16>(solver_param));
  ...
  if (gpus.size() > 1) {
    caffe::P2PSync<float16,float16> sync(solver, NULL, solver->param());
    sync.run(gpus);
  } else {
...
int test() {
  ...
  for (int i = 0; i < FLAGS_iterations; ++i) {
    ...
    for (int j = 0; j < result.size(); ++j) {
      ...
      for (int k = 0; k < result[j]->count(); ++k, ++idx) {
        const float16 score = Get<float16>(result_vec[k]);
        if (i == 0) {
  ...
  for (int i = 0; i < test_score.size(); ++i) {
    ...
    const float16 loss_weight = caffe_net.blob_loss_weights()[
        caffe_net.output_blob_indices()[test_score_output_id[i]]];
    std::ostringstream loss_msg_stream;
    const float16 mean_score = test_score[i] / FLAGS_iterations;
    if (loss_weight) {
